...
SELECT 
    issued_member_id, COUNT(*) AS Total_issued_book
FROM
    issued_status
...
CREATE TABLE book_issued_cnt AS SELECT b.isbn,
    b.book_title,
    COUNT(ist.issued_id) AS book_issued_cnt FROM
    issued_status ist
        JOIN
...
SELECT 
    e.emp_id, e.emp_name, e.position, e.salary, b.*
FROM
    employee e
        JOIN
...
CREATE TABLE Greater_rental_price AS SELECT * FROM
    books
WHERE
    rental_price >= (SELECT 
            AVG(rental_price)
...
FROM
    issued_status ist
        LEFT JOIN
    return_status re ON ist.issued_id = re.issued_id
WHERE
    re.return_id IS NULL;
...
FROM
    ...
    books b ON b.isbn = ist.issued_book_isbn
        LEFT JOIN
    return_status re ON re.issued_id = ist.issued_id
WHERE
...
CREATE TABLE branch_reports AS SELECT b.branch_id,
    ...
    issued_status ist
        JOIN
    employee e ON e.emp_id = ist.issued_emp_id
        JOIN
    branch b ON e.branch_id = b.branch_id
        LEFT JOIN
